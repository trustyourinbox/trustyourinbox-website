...
async function extractXmlBuffers(file: File): Promise<{ name: string; buffer: Buffer }[]> {
...
function humanizeDmarcXml(xml: any) {
  ...
  const records = (report.record || []).map((rec: any) => {
    ...
    return {
      ...
      dkim_domains: (authResults.dkim || []).map((d: any) => d.domain?.[0] || ''),
...
export async function POST(req: NextRequest) {
  ...
  for (const file of files) {
    try {
      ...
      results.push({ name: file.name, error: 'Failed to extract or decompress file.' });
    }
  }
